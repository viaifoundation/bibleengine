...
{
	$txt="\n【使用帮助】\n\n";
	$txt .="(接收#每日真道圣言#推送请关注微信号 Bible101 和基督徒百科 CCBible)\n\n";
...
function show_about()
{
	...
	$txt .= "\n\n我们的微信公众号：基督徒百科 CCBible 歌珊地 iGeshandi 服务号：歌珊地圣经引擎 BibleEngine ";
    $txt .= "\n\n我们的微博：@基督徒百科@歌珊地圣经引擎";
    $txt .= "\n\n我们的信望爱团契千人QQ群：福音群4619600　学习群226112909　团契群89857902　神学群81591230　百科群201463994　商务群226112700";
    $txt .= "\n\n详细帮助请参阅基督徒百科的歌珊地圣经引擎词条 https://baike.jidutu.org/wiki/BibleEngine\n";
	$txt .= "\n\n基督徒百科微信公众版词条 https://baike.jidutu.org/wiki/ccbible\n";
    $txt .= "\n\n联系方式：微博 @如鹰展翼而上 微信推特领英 michaelhuo 电邮 huo@live.cn QQ 38799316\n";
    $txt .= "\n请点击访问\n <a href=\"http://engine.geshandi.com\">圣经引擎微信触屏版</a>\n <a href=\"http://m.wsq.qq.com/263036651\">基督徒百科微社区</a>\n";
	$txt .=  show_hint() . show_banner();
	return $txt;
}
...
function show_helper()
{
	...
	$txt .= "\n我们的信条：威斯敏斯特信条（西敏信条） WCF，威斯敏斯特大要理问答（大要理问答）WLC，威斯敏斯特小要理问答（小要理问答）WSC";
    $txt .= "\n我们也认信：使徒信经 尼西亚信经 亚他那修信经 海德堡要理问答hc 比利时信条bc 多特法典cd 芝加哥圣经无误宣言";
	$txt .= "\n\n我们的微信公众号：基督徒百科 CCBible Bible101 歌珊地 iGeshandi 服务号：歌珊地圣经引擎 BibleEngine 歌珊地科技 iGoshen";
    $txt .= "\n\n我们的微博：@基督徒百科@歌珊地圣经引擎@Bible101";
    $txt .= "\n\n我们的信望爱团契千人QQ群：福音群4619600　学习群226112909　团契群89857902　神学群81591230　百科群201463994　商务群226112700";
    $txt .= "\n\n详细帮助请参阅基督徒百科的歌珊地圣经引擎词条 http://bible.world/BibleEngine\n";
	$txt .= "\n\n基督徒百科微信公众版词条 http://bible.world/ccbible\n";
    $txt .= "\n\n联系方式：微博 @如鹰展翼而上 微信 michaelhuo 电邮 huo@live.cn QQ 38799316\n";
    $txt .= "\n请点击访问\n <a href=\"http://engine.geshandi.com\">圣经引擎微信触屏版</a>\n <a href=\"http://m.wsq.qq.com/263036651\">基督徒百科微社区</a>\n";
	$txt .=  show_hint() . show_banner();
	return $txt;
}
...
//define your token
define("TOKEN", "geshandi");
$encodingAesKey = "WoaIUC3QGhdvDJ6ikh0QWvbyyJu75vToYXuBZRFthgR";
$token = "geshandi";
$timeStamp = "1409304348";
$nonce = "xxxxxx";
$appId = "wx47c647e000150c81"; //CCBible
$appId = "wx31df7509da616078"; //BibleEngine
$appId = "wx7d8a9e1689401e08"; //iGoshen
		...
		if (!empty($postStr)){
				...
				if($msgType=="event")
				{
					...
					if($keyword == "subscribe")
					{
						...
						$contentStr= $txt . show_hint() . show_banner();
					}else  if($keyword == "unsubscribe"){
						$txt=save_user($fromUsername, false);
						$contentStr="\n感谢关注" . show_hint() . show_banner();
					}else if($keyword =="CLICK"){
						if($EventKey=="HELP"){
							$contentStr=show_help();
						}
						else if($EventKey=="EXAMPLE"){
							$contentStr=show_example();
						}
						else if($EventKey=="ABOUT"){
							$contentStr=show_about();
						}
						else if($EventKey=="TIPS"){
							$contentStr=show_tips();
						}
                        else if($EventKey=="HELPER"){
							$contentStr=show_helper();
						}
                        else{
							$contentStr="感谢联系我们 " . show_hint() . show_banner();
                        }
						
                    }else{
						$contentStr="感谢联系我们，该操作还未支持。" . show_hint() . show_banner();
                    }
					//$contentStr = $keyword . "\n" . show_help();
				...
				else if($msgType=="text")
                {
					...
					//{ 
						...
						if($keyword=="?" || $keyword=="H" || $keyword=="h" || $keyword=="1")
						{
							$contentStr=show_help();
						}
						else if($keyword=="!" || $keyword=="2"  || $keyword=="E" || $keyword=="e")
						{
							$contentStr=show_example();
						}
               			else if($keyword=="#" || $keyword=="3"  || $keyword=="T" || $keyword=="t")
						{
							$contentStr=show_tips();
						}
     
                    	else if($keyword=="@" || $keyword=="9"  || $keyword=="A" || $keyword=="a")
						{
							$contentStr=show_about();
						}
                    	else if($keyword=="$" || $keyword=="0"  || $keyword=="B" || $keyword=="b")
						{
							$contentStr=show_helper();
						}	                    
							...
							$contentStr = $logStr . show_hint() . show_banner();
						}else if(substr($keyword,0,2)=="#="){
							$keyword=substr($keyword,2);
							$contentStr=show_debug($keyword,false);
						}else if(substr($keyword,0,2)=="#+"){
							$keyword=substr($keyword,2);
							$contentStr=show_debug($keyword,true);
						}else if(is_numeric($keyword)){
							$contentStr="感谢联系我们，该操作还未支持。" . show_hint() . show_banner();                         
						}else if(substr($keyword,0,1)=="!"){
							$keyword = substr($keyword,1);
							preg_match("/@([0-9]*)/", $keyword, $option);
							//print_r($query_option_array);
							$page=(int)$option[1];
							//ltrim($query_options, "@");
							$keyword=preg_replace("/@.*/","",$keyword);
							//$contentStr = $keyword . " " . $page;
							$contentStr = search_wiki($keyword,$page);
						}else{
                	...
                	$contentStr="\n可文本发送搜索关键词或经文章节号查询检索，欢迎您的意见或改进建议" . show_hint() . show_banner();
				//	echo $contentStr;
                }//if($msgType==)
...
function save_log($MsgID,$fromUsername,$toUsername,$time,$msgType,$keyword)
{
	...
	$db = mysqli_connect($dbhost_w.":".$dbport, $dbuser, $dbpassword);
	if(!$db)
...
function search_wiki($q,$p=1)
{
	...
	if(!$txt)
		$txt = "没有查到搜索的词条，请更换关键词再搜索。欢迎点击<a href=\"http://m.wsq.qq.com/263036651\">基督徒百科微社区</a>问答。"  . show_hint() . show_banner();
	return $txt ;
	//return "OK";
}
...
preg_match("/#([0-9]*)/",$query, $query_numbers);
$query=preg_replace("/#[0-9]*/","",$query);
    $query_number=$query_numbers[1];
    if(!isset($query_number) || !$query_number)
        $query_number=1;
...
if($book==100)
{
    return search_wiki($query,$query_number);
}
...
$db = mysqli_connect($dbhost.":".$dbport, $dbuser, $dbpassword) or die("Connection Error: " . mysqli_error());
...
//					echo $querystr;
	...
	//search
		...
		if($count > 10){
			$echo_string ="至多10个关键词，减少关键词以降低服务器的开销。";
			$do_query =false;
		}
		...
		{	
			...
			if($querystr){ //there are result
				...
				$index = '';
				$echo_string .= "没有查到记录，请修改搜索条件重新搜索，可查询书卷章节也可查询多关键词，空格分隔。欢迎点击<a href=\"http://m.wsq.qq.com/263036651\">基督徒百科微社区</a>问答。";
			}
