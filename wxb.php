...
function show_help()
{
	...
	$txt .= "\n可选择书卷范围 例如“神 说 @创” 或 “神 说 @出-利” 或“神 说 @2-5”（支持所有中英文简繁体书卷名和缩写格式和数字序号），需要将这个选项放在最后\n";
	$txt .= "\n可使用斜线设定选项，例如 /E /EN 英文 /C /CN 简体中文 /T /TW 繁体中文 /P /PINYIN 拼音译本 /NCVS /LCVS /BBE /KJV1611 /UKJV等译本, /KJV,/CUVS,/CUVT,/NASB含原文编码\n";
	$txt .= "欢迎提供改进意见和建议以便我们升级改进，您可以直接回复消息或者微博联系@基督徒百科 ccbible";
	$txt .=  "\n @歌珊地圣经引擎 ";
	return $txt;
}
function search($q)
...
foreach ($query_option_array as $query_option)
{
	switch(strtolower($query_option)){
		...
		case "h":case "?":case "help":case "帮助": $help=1;break;
	}
		...
		{	
			...
			//foreach($result as $row){
				...
				if($i >= $max_record_count){
					$echo_string .= "结果超出 $max_record_count 条记录，请增加关键词或者设定查询范围来准确查询\n使用 /HELP 选项获得帮助。可选择书卷范围 例如“神 说 @创” 或 “神 说 @出-利” 或“神 说 @2-5”（支持所有中英文简繁体书卷名和缩写格式和数字序号）\n 欢迎提供改进意见和建议以便我们升级改进，您可以直接回复消息或者微博联系@基督徒百科 ccbible";
			...
			if($querystr){ //there are result
				...
				$index = '';
				$echo_string .= "没有查到记录，请修改搜索条件重新搜索，可查询书卷章节也可查询多关键词，使用 /HELP 选项获得帮助。空格分隔，可选择书卷范围 例如“神 说 @创” 或 “神 说 @出-利” 或“神 说 @2-5”（支持所有书卷名格式和数字序号）\n欢迎提供改进意见和建议以便我们升级改进，您可以直接回复消息或者微博联系@基督徒百科 ccbible";
			}
