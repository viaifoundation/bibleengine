<?php
/**
 * Bible book name utilities
 * Centralized book name arrays and lookup functions
 */
...
// Short names for verse references: three locales, all 66 books. Use by translation language.
// en    => $book_short (e.g. Gen, Ezek, Rev)
// zh_cn => $book_cn    (Simplified, e.g. 创, 结, 启)
// zh_tw => $book_tw    (Traditional, e.g. 創, 結, 啟)
...
// Book index for lookup (includes all variations)
$book_index = [
 ...
 * @param string $book_name Book name in any format
 * @return int Book ID (1-66), 0 if not found
 */
function getBookId(string $book_name): int {
    global $book_index;
    $book_name = trim($book_name);
    return $book_index[$book_name] ?? 0;
}

/**
 * Get book short name by ID
 * @param int $book_id Book ID (1-66)
 * @return string Short name (e.g., "Gen", "创")
 */
function getBookShort(int $book_id): string {
    global $book_short;
 ...
 */
function getBookChapterCount(int $book_id): int {
    global $book_count;
    return $book_count[$book_id] ?? 0;
}
