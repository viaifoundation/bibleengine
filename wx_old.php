		...
		foreach($segments as $segment){
			...
			if($sql_where && !$echo_string)
			{
				...
				//foreach($result as $row){
					...
					if($count>500)
					{
						$echo_string="索引经文章节超过 500 条，请缩小范围后重新索引";
						//$querystr ="";
						break;
					}
				}
	...
	//search
		...
		{	
			...
			if($querystr){ //there are result
				...
				$index = '';
				$echo_string .= "没有查到记录，请修改搜索条件重新搜索，可查询书卷章节也可查询多关键词，空格分隔，可选择书卷范围 例如“神 说 @创” 或 “神 说 @出-利” 或“神 说 @2-5”（支持所有书卷名格式和数字序号）\n欢迎提供改进意见和建议以便我们升级改进，您可以直接回复消息或者微博联系@基督徒百科 ccbible";
			}
