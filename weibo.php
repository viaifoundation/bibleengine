...
require_once 'weibosdk.php';
//require_once 'biblesdk.php';

//$bible = new BibleEngine();

//设置app_key对应的app_secret

define("APP_SECRET", "d21f28b515a974c509ed79d2339e666c");
...
if (!empty($post_msg_str)) {
    ...
    $msgType = $post_msg_str['type'];
    $keyword = $post_msg_str['text'];
    
    if($msgType=="event"){
 		$msgData = $post_msg_str['data'];
        $subtype = $msgData['subtype'];
        if($subtype == "subscribe"){
             $contentStr="订阅成功，欢迎订阅，愿上帝赐福！($receiver_id)";
        }else if($subtype == "unsubscribe"){
             $contentStr="退订成功，感谢订阅，愿上帝赐福！($receiver_id)";
        }else if($subtype == "follow"){
             $contentStr="欢迎关注，愿上帝赐福！($receiver_id)";
        }else if($subtype == "unfollow"){
             $contentStr="谢谢关注，愿上帝赐福！($receiver_id)";
        }else{
        	$contentStr="感谢联系，愿上帝赐福！($receiver_id)";
        }
    }else if($msgType=="text" || $msgType=="mention"){
        //$MsgID = 0;
            ...
            if($receiver_id=="2764142363"){
                ...
                $keyword=str_replace("@圣经科技","",$keyword);
						...
						if($keyword=="?" || $keyword=="H" || $keyword=="h" || $keyword=="1")
						{
                            ...
                            //$contentStr="\n帮助：可文本发送搜索关键词或经文章节号查询检索，欢迎您的意见或改进建议";
						}
						else if($keyword=="!" || $keyword=="2"  || $keyword=="E" || $keyword=="e")
						{
                            $contentStr=show_example();
                            //$contentStr="\n示例：可文本发送搜索关键词或经文章节号查询检索，欢迎您的意见或改进建议";
						}
                    	else if($keyword=="@" || $keyword=="0"  || $keyword=="A" || $keyword=="a")
						{
                            $contentStr=show_about();
                            //$contentStr="\n关于：可文本发送搜索关键词或经文章节号查询检索，欢迎您的意见或改进建议";
						}						
						else if(substr($keyword,0,1)=="?"){
                            $contentStr="感谢联系我们，该操作还未支持。" . show_hint() . show_banner();  
                            //$contentStr="\n问题：可文本发送搜索关键词或经文章节号查询检索，欢迎您的意见或改进建议";
        ...
        //$contentStr="\n可文本发送搜索关键词或经文章节号查询检索，欢迎您的意见或改进建议" . $bible->show_hint() . $bible->show_banner();
				//	echo $contentStr;
    }//if($msgType==)
    //$contentStr .= $msgType . " " . $sender_id  . " " . $receiver_id . "\n\n" . $keyword;
    $data = $call_back_SDK->textData($contentStr);
...
{
	$txt="\n【关于我们】\n\n基督徒百科圣经引擎粉丝服务平台\n\n";
    $txt .="\n 圣经引擎——给力的圣经研读查经和圣经搜索引擎\n " . (isset($engine_name_en) ? $engine_name_en : 'Bible Engine') . " -- Powerful Agentic Bible Study and Search Engine ";
...
function search_wiki($q,$p=1)
{
	...
	if(mb_strlen($txt,'UTF-8')>290){
		$txt=mb_substr($txt,0,290,'UTF-8') ;
		$txt .= "\n\n太长有删节\n";	
	}
