...
$long_url_base="http://bible.godwithus.cn/bible";
	...
	//search
		...
		{	
			...
			foreach($result as $row){
				...
				$i ++;
				if($i > 200){
					$echo_string = "<h2>结果超出200条记录，请增加关键词或者设定查询范围来准确查询</h2>";
					break;
				}
			}
			$querystr = rtrim($querystr,",");
...
$title = "CCBible.me ";
if($mode=='VIEW')
	 $title .= "View 查看 <b>" . $english_title . "</b>";
elseif($mode=='INDEX')
	$title .= "Index 索引 <b>$query</b> ";
else
	$title .= "Search 搜索 <b>$query</b> ";
	
$title .= " -- Godwithus Bible 神同在圣经，好用的中文在线圣经引擎";
$logo= "/banner.png";
...
<p>&nbsp;</p>
  <p>神同在圣经 Godwithus Bible <a href="http://bible.godwithus.cn">http://bible.godwithus.cn</a> <a href="http://ccbible.sinaapp.com">http://ccbible.sinaapp.com</a> <br/>短网址 Short URL:  <a href="http://ccbible.me">http://ccbible.me</a> <a href="http://ccbible.us">http://ccbible.us</a> 我们的华人基督徒圣经 Chinese Christian Bible. 
    <br/> [<a href="https://docs.google.com/spreadsheet/viewform?formkey=dHhyYnZHMXI3N1pZLUExVEJxcVEzaXc6MQ" target=_blank>使用反馈调查 Feedback</a>]</p>


	<FORM method = "GET" action = "<?php echo $script?>"> 
<input type="text" size="80" maxlength="128" name="q" value="<?php echo $query ?>"> 
<input  type="submit" value="圣经搜索 Search"> 
...
<input type='checkbox' name='strong' value='strong' <?php if($strong) echo 'checked'?>>原文编号 Strong's*
...
<br/>
<input type='checkbox' name='cuvc' value='cuvc' <?php if($cuvc) echo 'checked'?>>文理和合本CUVC
<input type='checkbox' name='pinyin' value='pinyin' <?php if($pinyin) echo 'checked'?>>拼音和合本pinyin
<input type='checkbox' name='ncvs' value='ncvs' <?php if($ncvs) echo 'checked'?>>简体新译本NCVS
<input type='checkbox' name='lcvs' value='lcvs' <?php if($cuvs) echo 'checked'?>>吕振中译本LCVS
...
<input type='checkbox' name='ukjv' value='ukjv' <?php if($ukjv) echo 'checked'?>>更新钦定本UKJV
<input type='checkbox' name='kjv1611' value='kjv1611' <?php if($kjv1611) echo 'checked'?>>钦定本1611 KJV1611
<input type='checkbox' name='bbe' value='bbe' <?php if($bbe) echo 'checked'?>>简易英文译本BBE
...
for($i=1;$i<=66;++$i) {
	...
	$book_menu = $book_menu . "&nbsp;  <a href=\"$short_url_base/Bible." . $book_short[$i] . ".html\" title=\"" .  $book_chinese[$i] . " (" . $book_english[$i]  . ") \">" . $book_cn[$i] . " (" . $book_short[$i]. ") </a> ";
	if($book == $i)
	{
			$book_menu = $book_menu . "</strong>";
	}
...
for($i = 1; $i <= $book_count[$book]; $i++)
{
	...
	$chapter_menu = $chapter_menu . "<a href=\"$short_url_base/Bible." . $book_short[$book] . "." . $i . ".html\" title=\"" .  $book_chinese[$book] . " " . $i . " &nbsp; " . $book_english[$book] . " " . $i ."\"> &nbsp;" . $i . " </a> ";
	if($chapter)
	{
...
if($index)
{
	...
	if($mode=='QUERY' && !$echo_string || $mode=='VIEW')
	{
		$sql = "SELECT bible_books.* ";
...
{	
	...
	foreach($result as $row){
		...
		//$txt_py = $row['txt_py'];
		if($vid == $verse && ($mode=='VIEW' || $mode=='INDEX'))
		{
...
//		$book_short = $row['short'];
		...
		need further work
		*/
		$text_tw = $text_tw . " <sup><a href=\"$short_url_base/Bible." . $osis . ".html\">" . $book_tw[$bid] . " " . $cid . ":" . $vid . "</a></sup> " .  $txt_tw . "\n"; 
		$text_cn = $text_cn . " <sup><a href=\"$short_url_base/Bible." . $osis . ".html\">" . $book_cn[$bid] . " " .$cid . ":" . $vid . "</a></sup> " .  $txt_cn . "\n";
		$text_en = $text_en . " <sup><a href=\"$short_url_base/Bible." . $osis . ".html\">" . $book_short[$bid] . " " . $cid . ":" . $vid . "</a></sup> " .  $txt_en . "\n";
		//$text_py = $text_py . " <sup><a href=\"Bible." . $osis . ".html\">" . $book_short[$bid] . " " . $cid . ":" . $vid . "</a></sup> " .  $txt_py . "\n";
		if($verse_number %2)
		...
		$text_cmp = $text_cmp . "<p>\n";
		$text_cmp = $text_cmp .  "<sup><a href=\"$short_url_base/Bible." . $osis . ".html\"";
...
/*
		//for pinyin
		$text_cmp = $text_cmp . "<p>\n";
		$text_cmp = $text_cmp .  "<sup><a href=\"Bible." . $osis . ".html\"";
...
*/		
		...
		/*
		$text_com = $text_cmp . " <p><small><a href=\"Bible." . $osis . ".html\"";
		$text_cmp = $text_cmp .   " title=\"" .  $book_english[$bid] . " " . $cid . ":" . $vid .  "\">";
...
//		$text_cmp = $text_cmp .  "<a href=\"http://ccwiki.org/wiki/Bible:" . $osis . "\" target=_blank>(OSIS)</a>\n";
		...
		//$text_cmp = $text_cmp .  "<a href=\"https://www.51zanmei.net/bibleindex-$bid-$cid.html\" target=_blank>赞美圣经</a> |\n";
...
<div id="godwithusBible" align="center"></div>
...
<script type="text/javascript" src="https://zhsw.org/bk/plugins/momo/momo.js"></script>
<?php } ?>

<p>&nbsp;</p>
<p>
<strong>读经研经灵修</strong> &nbsp; 
<a href="http://blb.org" target="_blank">Blue Letter Bible</a> |
<a href="http://rbc.org/odb/odb.shtml" target="_blank">Our Daily Bread</a> |
<a href="http://chinese-odb.org/zh-hans/" target="_blank">灵命日粮</a> |
<a href="http://www.youversion.com/" target="_blank">Youversion</a> |
<a href="http://www.youversion4chinese.com/" target="_blank">优训网读经</a> |
<a href="http://bible.is/" target="_blank">Bible.is语音圣经</a> |
<a href="http://rock.ccim.org/gsword/gbook/v" target="_blank">神剑万维圣经工具</a> |
<a href="http://bible.fhl.net" target="_blank">信望爱信仰圣经资源</a>
<a href="http://bible.fhl.net/new/sdict.html" target="_blank">原文词典</a> |
<a href="http://www.ccbiblestudy.org/" target="_blank">CCBibleStudy查经资料</a> |
<a href="http://www.esv.org/" target="_blank">ESV圣经资料</a> 
</p>
