...
$long_url_base="http://bible.godwithus.cn/$script";
		...
		foreach($segments as $segment){
			...
			for($ii=1;$ii<67;$ii++)
			{
				...
				foreach ($book_names as $book_name){
					...
					$pattern="/^$book_name$/i";
					//there must be a space between book name and chapter number
					$temp_string =  explode(" ",$segment);
					if((int)$temp_string[0])
						$temp_pattern= $temp_string[0] . " " . $temp_string[1];
					else
						$temp_pattern= $temp_string[0];
					if(preg_match($pattern,$temp_pattern))
					//if($pos !== false)
					{
						//echo $book_name;
						$book=$ii;
							...
							{ 
								//$verse = $r1;
									...
									if((int)$verses_temp[0])
									{
										...
										$sql_where .= " AND (((verse>=" . (int)$verses_temp[0] .  " +1-$extend ) AND (verse<="  . (int)$verses_temp[0] . " + $extend -1 ))";
										$index_temp=count($verses_temp);
										for($iii=1;$iii<$index_temp;$iii++)
										{
											//$sql_where .= " OR (verse=" . (int)$verses_temp[$iii] . ") ";
											$sql_where .= " OR ((verse>=" . (int)$verses_temp[$iii] . "+ 1 - $extend ) AND (verse<="  . (int)$verses_temp[$iii] . "- 1 + $extend )) ";
										}
										$sql_where .= ") ";
									}
									else
							...
							if(!$r2 && !$r4){  // John 1-3
								...
								if(count($verse_array_temp)>0)
								{
									...
									foreach ($verse_array_temp as $verse_temp)
									{
										...
										if((int)$verse2_temp)
										{
											$sql_where .= " OR (( verse >= " . (int)$verse1_temp . ") AND (verse <= " . (int)$verse2_temp . ")) ";
										}else{
...
//					echo $querystr;
	...
	//search
		...
		if($range2)
				$sql_where =  " book BETWEEN $range1 AND $range2 ";
		elseif($range1)
			$sql_where = " book = $range1";
		else
			$sql_where = " 1=1 ";			
...
if($index)
{
	...
	if($chapter)
	{
		...
		if($verse)
			if($verse2)
				$sql = $sql . " AND bible_books.verse >= $verse + 1 - $extend AND bible_books.verse <=$verse2 + $extend -1";
			else
				$sql = $sql . " AND bible_books.verse >= $verse + 1 - $extend AND bible_books.verse <=$verse + $extend - 1";
...
//		$text_cmp = $text_cmp .  "<a href=\"http://ccwiki.org/wiki/Bible:" . $osis . "\" target=_blank>(OSIS)</a>\n";
		...
		$option_name = "Youversion 优训读经(HTTPS)";
		$text_cmp .= "<option value=\"$option_url\">$option_name</option>\n";
		$quick_link_text .= "<a href=$option_url target=_blank>$option_name</a>&nbsp; ";
...
<p>&nbsp;</p>
  <p>神同在圣经 Godwithus Bible <a href="http://bible.godwithus.cn">http://bible.godwithus.cn</a> <a href="http://ccbible.me">http://ccbible.me</a> 我的华人基督徒圣经 Chinese Christian Bible for Me. 
	...
	<FORM method = "GET" action = "<?php echo $script?>" id="scripture_form"> 
<input type="text" size="80" maxlength="128" name="q" value="<?php echo $query ?>"> 
<input  type="submit" value="查经 GO!"  id="scripture_button"> 
<input type='checkbox' name='o' id='o' value='o' <?php if($options) echo 'checked'?> onChange="javascript:toggleOptions(this)">选项 Options
<a href="http://godwithus.cn/wiki/神同在圣经"> 帮助 Help</a> (<a href="devel.php">Devel</a> <a href="alpha.php">Alpha</a> <a href="beta.php">Beta</a> <a href="legacy.php">Legacy</a>)
...
<select name="s">
<option value=0 <?php if(!$scope) echo "SELECTED";?>>整本圣经 Whole Bible</option>
<option value="1-39" <?php if($scope=="1-39") echo "SELECTED";?>>旧约全书 Old Testament</option>
<option value="40-66" <?php if($scope=="40-66") echo "SELECTED";?>>新约全书 New Testament</option>
<option value="1-5" <?php if($scope=="1-5") echo "SELECTED";?>>摩西五经 Law (Gen - Deut)</option>
<option value="6-17" <?php if($scope=="6-17") echo "SELECTED";?>>历史书 History (Josh - Esth)</option>
<option value="18-22" <?php if($scope=="18-22") echo "SELECTED";?>>诗歌智慧书 Poetry & Wisdom (Job - Song)</option>
<option value="23-27" <?php if($scope=="23-27") echo "SELECTED";?>>大先知书 Major Prophets (Is - Dan)</option>
<option value="28-39" <?php if($scope=="28-39") echo "SELECTED";?>>小先知书 Minor Prophets (Hos - Mal)</option>
<option value="40-44" <?php if($scope=="40-44") echo "SELECTED";?>>福音历史书 Gospels (Matt - Acts)</option>
<option value="45-57" <?php if($scope=="45-57") echo "SELECTED";?>>保罗书信 Paul&#39;s Epistles (Rom - Philem)</option>
<option value="58-66" <?php if($scope=="58-66") echo "SELECTED";?>>一般使徒书信 General Epistles (Heb - Rev)</option>
<option value=1 <?php if($scope=="1") echo "SELECTED";?>>创世记 Genesis</option> 
<option value=2 <?php if($scope=="2") echo "SELECTED";?>>出埃及记 Exodus</option> 
<option value=3 <?php if($scope=="3") echo "SELECTED";?>>利未记 Leviticus</option> 
<option value=4 <?php if($scope=="4") echo "SELECTED";?>>民数记 Numbers</option> 
<option value=5 <?php if($scope=="5") echo "SELECTED";?>>申命记 Deuteronomy</option> 
<option value=6 <?php if($scope=="6") echo "SELECTED";?>>约书亚记 Joshua</option> 
<option value=7 <?php if($scope=="7") echo "SELECTED";?>>士师记 Judges</option> 
<option value=8 <?php if($scope=="8") echo "SELECTED";?>>路得记 Ruth</option> 
<option value=9 <?php if($scope=="9") echo "SELECTED";?>>撒母耳记上 1 Samuel</option> 
<option value=10 <?php if($scope=="10") echo "SELECTED";?>>撒母耳记下 2 Samuel</option> 
<option value=11 <?php if($scope=="11") echo "SELECTED";?>>列王记上 1 Kings</option> 
<option value=12 <?php if($scope=="12") echo "SELECTED";?>>列王记下 2 Kings</option> 
<option value=13 <?php if($scope=="13") echo "SELECTED";?>>历代志上 1 Chronicles</option> 
<option value=14 <?php if($scope=="14") echo "SELECTED";?>>历代志下 2 Chronicles</option> 
<option value=15 <?php if($scope=="15") echo "SELECTED";?>>以斯拉记 Ezra</option> 
<option value=16 <?php if($scope=="16") echo "SELECTED";?>>尼希米记 Nehemiah</option> 
<option value=17 <?php if($scope=="17") echo "SELECTED";?>>以斯帖记 Esther</option> 
<option value=18 <?php if($scope=="18") echo "SELECTED";?>>约伯记 Job</option> 
<option value=19 <?php if($scope=="19") echo "SELECTED";?>>诗篇 Psalms</option> 
<option value=20 <?php if($scope=="20") echo "SELECTED";?>>箴言 Proverbs</option> 
<option value=21 <?php if($scope=="21") echo "SELECTED";?>>传道书 Ecclesiastes</option> 
<option value=22 <?php if($scope=="22") echo "SELECTED";?>>雅歌 Song of Solomon</option> 
<option value=23 <?php if($scope=="23") echo "SELECTED";?>>以赛亚书 Isaiah</option> 
<option value=24 <?php if($scope=="24") echo "SELECTED";?>>耶利米书 Jeremiah</option> 
<option value=25 <?php if($scope=="25") echo "SELECTED";?>>耶利米哀歌 Lamentations</option> 
<option value=26 <?php if($scope=="26") echo "SELECTED";?>>以西结书 Ezekiel</option> 
<option value=27 <?php if($scope=="27") echo "SELECTED";?>>但以理书 Daniel</option> 
<option value=28 <?php if($scope=="28") echo "SELECTED";?>>何西阿书 Hosea</option> 
<option value=29 <?php if($scope=="29") echo "SELECTED";?>>约珥书 Joel</option> 
<option value=30 <?php if($scope=="30") echo "SELECTED";?>>阿摩司书 Amos</option> 
<option value=31 <?php if($scope=="31") echo "SELECTED";?>>俄巴底亚书 Obadiah</option> 
<option value=32 <?php if($scope=="32") echo "SELECTED";?>>约拿书 Jonah</option> 
<option value=33 <?php if($scope=="33") echo "SELECTED";?>>弥迦书 Micah</option> 
<option value=34 <?php if($scope=="34") echo "SELECTED";?>>那鸿书 Nahum</option> 
<option value=35 <?php if($scope=="35") echo "SELECTED";?>>哈巴谷书 Habakkuk</option> 
<option value=36 <?php if($scope=="36") echo "SELECTED";?>>西番雅书 Zephaniah</option> 
<option value=37 <?php if($scope=="37") echo "SELECTED";?>>哈该书 Haggai</option> 
<option value=38 <?php if($scope=="38") echo "SELECTED";?>>撒迦利亚书 Zechariah</option> 
<option value=39 <?php if($scope=="39") echo "SELECTED";?>>玛拉基书 Malachi</option> 
<option value=40 <?php if($scope=="40") echo "SELECTED";?>>马太福音 Matthew</option> 
<option value=41 <?php if($scope=="41") echo "SELECTED";?>>马可福音 Mark</option> 
<option value=42 <?php if($scope=="42") echo "SELECTED";?>>路加福音 Luke</option> 
<option value=43 <?php if($scope=="43") echo "SELECTED";?>>约翰福音 John</option> 
<option value=44 <?php if($scope=="44") echo "SELECTED";?>>使徒行传 Acts</option> 
<option value=45 <?php if($scope=="45") echo "SELECTED";?>>罗马书 Romans</option> 
<option value=46 <?php if($scope=="46") echo "SELECTED";?>>哥林多前书 1 Corinthians</option> 
<option value=47 <?php if($scope=="47") echo "SELECTED";?>>哥林多后书 2 Corinthians</option> 
<option value=48 <?php if($scope=="48") echo "SELECTED";?>>加拉太书 Galatians</option> 
<option value=49 <?php if($scope=="49") echo "SELECTED";?>>以弗所书 Ephesians</option> 
<option value=50 <?php if($scope=="50") echo "SELECTED";?>>腓立比书 Philippians</option> 
<option value=51 <?php if($scope=="51") echo "SELECTED";?>>歌罗西书 Colossians</option> 
<option value=52 <?php if($scope=="52") echo "SELECTED";?>>帖撒罗尼迦前书 1 Thessalonians</option> 
<option value=53 <?php if($scope=="53") echo "SELECTED";?>>帖撒罗尼迦后书 2 Thessalonians</option> 
<option value=54 <?php if($scope=="54") echo "SELECTED";?>>提摩太前书 1 Timothy</option> 
<option value=55 <?php if($scope=="55") echo "SELECTED";?>>提摩太后书 2 Timothy</option> 
<option value=56 <?php if($scope=="56") echo "SELECTED";?>>提多书 Titus</option> 
<option value=57 <?php if($scope=="57") echo "SELECTED";?>>腓利门书 Philemon</option> 
<option value=58 <?php if($scope=="58") echo "SELECTED";?>>希伯来书 Hebrews</option> 
<option value=59 <?php if($scope=="59") echo "SELECTED";?>>雅各书 James</option> 
<option value=60 <?php if($scope=="60") echo "SELECTED";?>>彼得前书 1 Peter</option> 
<option value=61 <?php if($scope=="61") echo "SELECTED";?>>彼得后书 2 Peter</option> 
<option value=62 <?php if($scope=="62") echo "SELECTED";?>>约翰壹书 1 John</option> 
<option value=63 <?php if($scope=="63") echo "SELECTED";?>>约翰贰书 2 John</option> 
<option value=64 <?php if($scope=="64") echo "SELECTED";?>>约翰叁书 3 John</option> 
<option value=65 <?php if($scope=="65") echo "SELECTED";?>>犹大书 Jude</option> 
<option value=66 <?php if($scope=="66") echo "SELECTED";?>>启示录 Revelation</option> 
</select> 
连续 Multi<select name="m">
<option value=0 <?php if($multi_verse==0) echo "SELECTED";?>>1</option>
<option value=3 <?php if($multi_verse==3) echo "SELECTED";?>>3</option>
</select>节 Verses
扩展 Extend<select name="e">
...
API <select name="a">
<option value="" <?php if($api=="") echo "SELECTED";?>>n/a</option>
<option value="json" <?php if($api=="json") echo "SELECTED";?>>json</option>
<option value="number" <?php if($api=="number") echo "SELECTED";?>>number</option>
<option value="text" <?php if($api=="text") echo "SELECTED";?>>text</option>
<option value="jsontext" <?php if($api=="jsontext") echo "SELECTED";?>>jsontext</option>
<option value="plain" <?php if($api=="plain") echo "SELECTED";?>>plain</option>
<option value="html" <?php if($api=="html") echo "SELECTED";?>>html</option>
</select>
API Lang<select name="l">
<option value="cn" <?php if($language=="cn") echo "SELECTED";?>>简CN</option>
<option value="tw" <?php if($language=="tw") echo "SELECTED";?>>繁TW</option>
<option value="en" <?php if($language=="en") echo "SELECTED";?>>英EN</option>
</select>
...
<br/>(<small><a href="http://godwithus.cn/wiki/圣经版权">圣经译本版权信息 Bible Translation Copyright Information</a></small>)
</div>
</FORM> 
