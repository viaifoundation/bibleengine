...
if($book==100)
{
    header("Location: https://baike.jidutu.org/w/index.php?title=Special%3A%E6%90%9C%E7%B4%A2&go=%E5%89%8D%E5%BE%80&search=$query");   
}
...
*/		
		...
		foreach ($bible_books as $bible_book) {
				if($bible_book)
				{
					...
					$text_cmp .= "\n<li><p>" . $text_string . "  (" . strtoupper($bible_book) . ") </p></li>\n";
				}
		}
		$text_cmp .= "</ul>\n";
...
//		$text_cmp = $text_cmp .  "<a href=\"http://ccwiki.org/wiki/Bible:" . $osis . "\" target=_blank>(OSIS)</a>\n";
		...
		$text_cmp .= "<option value=\"$option_url\">$option_name</option>\n";
		$quick_link_text .= "<a href=$option_url target=_blank>$option_name</a>&nbsp; ";
		$option_url = "https://www.bible.com/bible/47/" .  $book_short[$bid] . "." . $cid  . "." . $vid ;
		$option_name = "优训读经";
		$text_cmp .= "<option value=\"$option_url\">$option_name</option>\n";
		$quick_link_text .= "<a href=$option_url target=_blank>$option_name</a>&nbsp; ";
